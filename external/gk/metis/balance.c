...
void Bnd2WayBalance(ctrl_t* ctrl, graph_t* graph, real_t* ntpwgts) {
    ...
    IFSET(ctrl->dbglvl, METIS_DBG_REFINE, printf("Partitions: [%6" PRIDX " %6" PRIDX "] T[%6" PRIDX " %6" PRIDX "], Nv-Nb[%6" PRIDX " %6" PRIDX "]. ICut: %6" PRIDX " [B]\n", pwgts[0], pwgts[1], tpwgts[0], tpwgts[1], graph->nvtxs, graph->nbnd, graph->mincut));
    ...
    for(nswaps = 0; nswaps < nvtxs; nswaps++) {
        ...
        IFSET(ctrl->dbglvl, METIS_DBG_MOVEINFO, printf("Moved %6" PRIDX " from %" PRIDX ". [%3" PRIDX " %3" PRIDX "] %5" PRIDX " [%4" PRIDX " %4" PRIDX "]\n", higain, from, ed[higain] - id[higain], vwgt[higain], mincut, pwgts[0], pwgts[1]));
    ...
    IFSET(ctrl->dbglvl, METIS_DBG_REFINE, printf("\tMinimum cut: %6" PRIDX ", PWGTS: [%6" PRIDX " %6" PRIDX "], NBND: %6" PRIDX "\n", mincut, pwgts[0], pwgts[1], nbnd));
...
void McGeneral2WayBalance(ctrl_t* ctrl, graph_t* graph, real_t* ntpwgts) {
    ...
    if(ctrl->dbglvl & METIS_DBG_REFINE) {
        printf("Parts: [");
        for(l = 0; l < ncon; l++)
            printf("(%6" PRIDX " %6" PRIDX " %.3" PRREAL " %.3" PRREAL ") ", pwgts[l], pwgts[ncon + l], ntpwgts[l], ntpwgts[ncon + l]);
