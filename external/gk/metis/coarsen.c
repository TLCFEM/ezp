                    ...
                    if(ncon == 1) {
                        ...
                        for(j = xadj[i]; j < xadj[i + 1]; j++) {
                            ...
                            if(match[k] == UNMATCHED &&
                               ...
                               (maxwgt < adjwgt[j] ||
                                (maxwgt == adjwgt[j] &&
                                 BetterVBalance(ncon, graph->invtvwgt, vwgt + i * ncon, vwgt + maxidx * ncon, vwgt + k * ncon)))) {
                                maxidx = k;
                                maxwgt = adjwgt[j];
                            }
                        }
...
void PrintCGraphStats(ctrl_t* ctrl, graph_t* graph) {
    ...
    printf("%10" PRIDX " %10" PRIDX " %10" PRIDX " [%" PRIDX "] [", graph->nvtxs, graph->nedges, isum(graph->nedges, graph->adjwgt, 1), ctrl->CoarsenTo);
