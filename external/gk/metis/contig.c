...
void EliminateComponents(ctrl_t* ctrl, graph_t* graph) {
    ...
    IFSET(ctrl->dbglvl, METIS_DBG_CONTIGINFO, printf("I found %" PRIDX " components, for this %" PRIDX "-way partition\n", ncmps, nparts));
    ...
    if(ncmps > nparts) {
        ...
        while(ntodo > 0) {
            ...
            for(i = 0; i < ntodo; i++) {
                ...
                IFSET(ctrl->dbglvl, METIS_DBG_CONTIGINFO, printf("Trying to move %" PRIDX " [%" PRIDX "] from %" PRIDX "\n", cid, isum(ncon, cwgt, 1), me));
                ...
                target = cand[0].val;
                for(j = 1; j < ncand; j++) {
                    if(BetterBalanceKWay(ncon, cwgt, ctrl->ubfactors, 1, pwgts + target * ncon, ctrl->pijbm + target * ncon, 1, pwgts + cand[j].val * ncon, ctrl->pijbm + cand[j].val * ncon))
                        target = cand[j].val;
                }

                IFSET(ctrl->dbglvl, METIS_DBG_CONTIGINFO, printf("\tMoving it to %" PRIDX " [%" PRIDX "] [%" PRIDX "]\n", target, cpvec[target], ncand));
                ...
                if(target != me) {
                    ...
                    case METIS_OBJTYPE_VOL:
                        MoveGroupContigForVol(ctrl, graph, target, cid, cptr, cind, vmarker, pmarker, modind);
                        break;
...
void MoveGroupContigForCut(ctrl_t* ctrl, graph_t* graph, idx_t to, idx_t gid, idx_t* ptr, idx_t* ind) {
    ...
    for(iii = ptr[gid]; iii < ptr[gid + 1]; iii++) {
        ...
        for(j = xadj[i]; j < xadj[i + 1]; j++) {
            ...
            UpdateAdjacentVertexInfoAndBND(ctrl, ii, xadj[ii + 1] - xadj[ii], me, from, to, myrinfo, adjwgt[j], nbnd, bndptr, bndind, BNDTYPE_REFINE);
        }
...
void MoveGroupContigForVol(ctrl_t* ctrl, graph_t* graph, idx_t to, idx_t gid, idx_t* ptr, idx_t* ind, idx_t* vmarker, idx_t* pmarker, idx_t* modind) {
    ...
    ASSERTP(ComputeVolume(graph, where) == graph->minvol, ("%" PRIDX " %" PRIDX "\n", ComputeVolume(graph, where), graph->minvol));
}
