...
void Print2WayRefineStats(ctrl_t* ctrl, graph_t* graph, real_t* ntpwgts, real_t deltabal, idx_t mincutorder) {
    ...
    if(mincutorder == -2) {
        ...
        for(i = 0; i < graph->ncon; i++)
            printf("(%.3" PRREAL " %.3" PRREAL " T:%.3" PRREAL " %.3" PRREAL ")", graph->pwgts[i] * graph->invtvwgt[i], graph->pwgts[graph->ncon + i] * graph->invtvwgt[i], ntpwgts[i], ntpwgts[graph->ncon + i]);
        ...
        for(i = 0; i < graph->ncon; i++)
            printf("(%.3" PRREAL " %.3" PRREAL ")", graph->pwgts[i] * graph->invtvwgt[i], graph->pwgts[graph->ncon + i] * graph->invtvwgt[i]);
        printf("] LB: %.3" PRREAL "(%+.3" PRREAL ")\n", ComputeLoadImbalance(graph, 2, ctrl->pijbm), deltabal);
    }
}
