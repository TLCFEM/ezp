...
idx_t MlevelKWayPartitioning(ctrl_t* ctrl, graph_t* graph, idx_t* part) {
    ...
    for(i = 0; i < ctrl->ncuts; i++) {
        ...
        IFSET(ctrl->dbglvl, METIS_DBG_IPART, printf("Initial %" PRIDX "-way partitioning cut: %" PRIDX "\n", ctrl->nparts, objval));
...
void InitKWayPartitioning(ctrl_t* ctrl, graph_t* graph) {
    ...
    case METIS_OBJTYPE_VOL:
        ...
        status = METIS_PartGraphRecursive(&graph->nvtxs, &graph->ncon, graph->xadj, graph->adjncy, graph->vwgt, graph->vsize, graph->adjwgt, &ctrl->nparts, ctrl->tpwgts, ubvec, options, &curobj, graph->where);
