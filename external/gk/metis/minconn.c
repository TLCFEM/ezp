...
void EliminateSubDomainEdges(ctrl_t* ctrl, graph_t* graph) {
    ...
    while(1) {
        ...
        while((me = ipqGetTop(&queue)) != -1) {
            ...
            IFSET(ctrl->dbglvl, METIS_DBG_CONNINFO, printf("Me: %" PRIDX ", Degree: %4" PRIDX ", TotalOut: %" PRIDX ",\n", me, nads[me], totalout));
            ...
            for(scheme = 0; scheme < 2; scheme++) {
                for(min = 0; min < ncand2; min++) {
                    ...
                    IFSET(ctrl->dbglvl, METIS_DBG_CONNINFO, printf("\tMinOut: %4" PRIDX ", to: %3" PRIDX ", TtlWgt: %5" PRIDX "[#:%" PRIDX "]\n", mypmat[other], other, isum(ncon, cpwgt, 1), nind));
                    ...
                    if(target != -1) {
                        IFSET(ctrl->dbglvl, METIS_DBG_CONNINFO, printf("\t\tScheme: %" PRIDX ". Moving to %" PRIDX "\n", scheme, target));
                        move = 1;
                        break;
                    }
                }
            ...
            if(target != -1) {
                ...
                case METIS_OBJTYPE_CUT:
                    ...
                    break;
                case METIS_OBJTYPE_VOL:
                    MoveGroupMinConnForVol(ctrl, graph, target, nind, ind, vmarker, pmarker, modind);
                    break;
                default:
