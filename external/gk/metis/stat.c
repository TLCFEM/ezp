...
void ComputePartitionInfoBipartite(graph_t* graph, idx_t nparts, idx_t* where) {
    ...
    if(ncon == 1) {
        printf("\tBalance: %5.3" PRREAL " out of %5.3" PRREAL "\n", 1.0 * nparts * kpwgts[iargmax(nparts, kpwgts, 1)] / (1.0 * isum(nparts, kpwgts, 1)), 1.0 * nparts * vwgt[iargmax(nvtxs, vwgt, 1)] / (1.0 * isum(nparts, kpwgts, 1)));
    }
    else {
        printf("\tBalance:");
        for(j = 0; j < ncon; j++)
            printf(" (%5.3" PRREAL " out of %5.3" PRREAL ")", 1.0 * nparts * kpwgts[ncon * iargmax_strd(nparts, kpwgts + j, ncon) + j] / (1.0 * isum(nparts, kpwgts + j, ncon)), 1.0 * nparts * vwgt[ncon * iargmax_strd(nvtxs, vwgt + j, ncon) + j] / (1.0 * isum(nparts, kpwgts + j, ncon)));
        printf("\n");
    }
    ...
    printf("Min/Max/Avg/Bal # of adjacent     subdomains: %5" PRIDX " %5" PRIDX " %5" PRIDX " %7.3" PRREAL "\n", kpwgts[iargmin(nparts, kpwgts, 1)], kpwgts[iargmax(nparts, kpwgts, 1)], isum(nparts, kpwgts, 1) / nparts, 1.0 * nparts * kpwgts[iargmax(nparts, kpwgts, 1)] / (1.0 * isum(nparts, kpwgts, 1)));
    ...
    printf("Min/Max/Avg/Bal/Frac # of interface    nodes: %5" PRIDX " %5" PRIDX " %5" PRIDX " %7.3" PRREAL " %7.3" PRREAL "\n", kpwgts[iargmin(nparts, kpwgts, 1)], kpwgts[iargmax(nparts, kpwgts, 1)], isum(nparts, kpwgts, 1) / nparts, 1.0 * nparts * kpwgts[iargmax(nparts, kpwgts, 1)] / (1.0 * isum(nparts, kpwgts, 1)), 1.0 * isum(nparts, kpwgts, 1) / (1.0 * nvtxs));
