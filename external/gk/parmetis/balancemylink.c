 ...
 **************************************************************************/
idx_t BalanceMyLink(ctrl_t* ctrl, graph_t* graph, idx_t* home, idx_t me, idx_t you, real_t* flows, real_t maxdiff, real_t* diff_cost, real_t* diff_lbavg, real_t avgvwgt) {
    ...
    for(pass = 0; pass < N_MOC_BAL_PASSES; pass++) {
        ...
        for(ii = 0; ii < nvtxs / 2; ii++) {
            from = -1;
            Mc_DynamicSelectQueue(ctrl, nqueues, ncon, me, you, inq, flows, &from, &qnum, minval, avgvwgt, maxdiff);
            ...
            for(j = xadj[vtx]; j < xadj[vtx + 1]; j++) {
                ...
                if(myqueue[edge] != -1) {
                    ...
                    rpqUpdate(queues[hval[edge] + (nqueues * myqueue[edge])], map[edge] - ptr[hval[edge]], newgain);
                }
            }
        }
