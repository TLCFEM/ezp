...
void CommSetup(ctrl_t* ctrl, graph_t* graph) {
    ...
    gkMPI_Alltoall((void*)recvrequests, 2, IDX_T, (void*)sendrequests, 2, IDX_T, ctrl->comm);
    ...
    for(i = 0; i < nnbrs; i++) {
        gkMPI_Irecv((void*)(sendind + sendptr[i]), sendptr[i + 1] - sendptr[i], IDX_T, peind[i], 1, ctrl->comm, ctrl->rreq + i);
    }

    /* Issue the sends. My recvind[penum] becomes penum's sendind[mype] */
    for(i = 0; i < nnbrs; i++) {
        gkMPI_Isend((void*)(recvind + recvptr[i]), recvptr[i + 1] - recvptr[i], IDX_T, peind[i], 1, ctrl->comm, ctrl->sreq + i);
    }
    ...
    for(i = 0; i < nsend; i++) {
        PASSERTP(ctrl, sendind[i] >= firstvtx && sendind[i] < lastvtx, (ctrl, "%" PRIDX " %" PRIDX " %" PRIDX "\n", sendind[i], firstvtx, lastvtx));
    ...
    rprintf(ctrl, "[%5" PRIDX " %5" PRIDX "] \tl:[%5" PRIDX " %5" PRIDX "] \ts:[%5" PRIDX ", %5" PRIDX "] \tr:[%5" PRIDX ", %5" PRIDX "]\n", GlobalSEMin(ctrl, nvtxs), GlobalSEMax(ctrl, nvtxs), GlobalSEMin(ctrl, nlocal), GlobalSEMax(ctrl, nlocal), GlobalSEMin(ctrl, nsend), GlobalSEMax(ctrl, nsend), GlobalSEMin(ctrl, nrecv), GlobalSEMax(ctrl, nrecv));
...
void CommInterfaceData(ctrl_t* ctrl, graph_t* graph, idx_t* data, idx_t* recvvector) {
    ...
    for(i = 0; i < nnbrs; i++) {
        gkMPI_Irecv((void*)(recvvector + recvptr[i]), recvptr[i + 1] - recvptr[i], IDX_T, peind[i], 1, ctrl->comm, ctrl->rreq + i);
    }
    ...
    for(i = 0; i < nnbrs; i++) {
        gkMPI_Isend((void*)(sendvector + sendptr[i]), sendptr[i + 1] - sendptr[i], IDX_T, peind[i], 1, ctrl->comm, ctrl->sreq + i);
    }
...
void CommChangedInterfaceData(ctrl_t* ctrl, graph_t* graph, idx_t nchanged, idx_t* changed, idx_t* data, ikv_t* sendpairs, ikv_t* recvpairs) {
    ...
    /* Issue the receives first */
    for(i = 0; i < nnbrs; i++) {
        gkMPI_Irecv((void*)(recvpairs + recvptr[i]), 2 * (recvptr[i + 1] - recvptr[i]), IDX_T, peind[i], 1, ctrl->comm, ctrl->rreq + i);
    }
    ...
    if(nchanged != 0) {
        ...
        for(i = 0; i < nnbrs; i++) {
            gkMPI_Isend((void*)(sendpairs + sendptr[i]), 2 * (psendptr[i] - sendptr[i]), IDX_T, peind[i], 1, ctrl->comm, ctrl->sreq + i);
        }
