...
void PrintVector2(ctrl_t* ctrl, idx_t n, idx_t first, idx_t* vec, char* title) {
    ...
    for(penum = 0; penum < ctrl->npes; penum++) {
        if(ctrl->mype == penum) {
            ...
            for(i = 0; i < n; i++)
                printf("[%" PRIDX " %" PRIDX ".%" PRIDX "] ", first + i, (idx_t)(vec[i] >= KEEP_BIT ? 1 : 0), (idx_t)(vec[i] >= KEEP_BIT ? vec[i] - KEEP_BIT : vec[i]));
            printf("\n");
            fflush(stdout);
        }
        gkMPI_Barrier(ctrl->comm);
 ...
 **************************************************************************/
void PrintTransferedGraphs(ctrl_t* ctrl, idx_t nnbrs, idx_t* peind, idx_t* slens, idx_t* rlens, idx_t* sgraph, idx_t* rgraph) {
