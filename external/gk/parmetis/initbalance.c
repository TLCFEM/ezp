...
void Balance_Partition(ctrl_t* ctrl, graph_t* graph) {
    ...
    if(ctrl->ps_relation == PARMETIS_PSR_UNCOUPLED) {
        ...
        gkMPI_Allgatherv((void*)graph->home, graph->nvtxs, IDX_T, (void*)part, rcounts, rdispls, IDX_T, ctrl->comm);
    ...
    IFSET(ctrl->dbglvl, DBG_REFINEINFO, ComputeSerialBalance(ctrl, agraph, agraph->where, lbvec));
    IFSET(ctrl->dbglvl, DBG_REFINEINFO, rprintf(ctrl, "input cut: %" PRIDX ", balance: ", ComputeSerialEdgeCut(agraph)));
    ...
    if(sr == 1) { /* Half of the processors do scratch-remap */
        ...
        while(lnpes > 1 && lnparts > 1) {
            ...
            METIS_PartGraphRecursive(&agraph->nvtxs, &ncon, agraph->xadj, agraph->adjncy, agraph->vwgt, NULL, agraph->adjwgt, &twoparts, tpwgts2, NULL, moptions, &edgecut, part);
        ...
        if(lnparts == 1) { /* Case in which srnpes is greater or equal to nparts */
            ...
            METIS_PartGraphKway(&agraph->nvtxs, &ncon, agraph->xadj, agraph->adjncy, agraph->vwgt, NULL, agraph->adjwgt, &lnparts, tpwgts, NULL, moptions, &edgecut, part);
        ...
        gkMPI_Allreduce((void*)&lpecost, (void*)&gpecost, 1, MPI_DOUBLE_INT, MPI_MINLOC, ipcomm);
        ...
        if(ncon == 1) {
            ...
            Mc_Diffusion(myctrl, agraph, graph->vtxdist, agraph->where, home, N_MOC_GD_PASSES);
        }
...
graph_t* AssembleAdaptiveGraph(ctrl_t* ctrl, graph_t* graph) {
    ...
    /**************************************/
    gkMPI_Allgatherv((void*)mygraph, mysize, IDX_T, (void*)ggraph, rcounts, rdispls, IDX_T, ctrl->comm);
