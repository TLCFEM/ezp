...
void InitMultisection(ctrl_t* ctrl, graph_t* graph) {
    ...
    METIS_ComputeVertexSeparator(&agraph->nvtxs, agraph->xadj, agraph->adjncy, agraph->vwgt, options, &agraph->mincut, agraph->where);
    ...
    if(myrank == 0 && myrank != gpecut.rank)
        gkMPI_Recv((void*)agraph->where, agraph->nvtxs, IDX_T, gpecut.rank, 1, newcomm, &ctrl->status);
    ...
    if(myrank == 0) {
        ...
        gkMPI_Reduce((void*)gwhere, (void*)part, graph->gnvtxs, IDX_T, MPI_SUM, 0, labelcomm);
    }
    ...
    gkMPI_Scatterv((void*)agraph->where, sendcounts, displs, IDX_T, (void*)graph->where, graph->nvtxs, IDX_T, 0, ctrl->comm);
...
graph_t* AssembleMultisectedGraph(ctrl_t* ctrl, graph_t* graph) {
    ...
    gkMPI_Allgatherv((void*)mygraph, mysize, IDX_T, (void*)ggraph, recvcounts, displs, IDX_T, ctrl->comm);
