 ...
 ************************************************************************************/
int ParMETIS_V3_PartKway(idx_t* vtxdist, idx_t* xadj, idx_t* adjncy, idx_t* vwgt, idx_t* adjwgt, idx_t* wgtflag, idx_t* numflag, idx_t* ncon, idx_t* nparts, real_t* tpwgts, real_t* ubvec, idx_t* options, idx_t* edgecut, idx_t* part, MPI_Comm* comm) {
    ...
    status = CheckInputsPartKway(vtxdist, xadj, adjncy, vwgt, adjwgt, wgtflag, numflag, ncon, nparts, tpwgts, ubvec, options, edgecut, part, comm);
    ...
    if(npes == 1) {
        ...
        status = METIS_PartGraphKway(&nvtxs, ncon, xadj, adjncy, vwgt, NULL, adjwgt, nparts, tpwgts, ubvec, moptions, edgecut, part);
    ...
    if(vtxdist[npes] < SMALLGRAPH || vtxdist[npes] < npes * 20 || GlobalSESum(ctrl, graph->nedges) == 0) { /* serially */
...
void Global_Partition(ctrl_t* ctrl, graph_t* graph) {
    ...
    if(ctrl->dbglvl & DBG_PROGRESS) {
        rprintf(ctrl, "[%6" PRIDX " %8" PRIDX " %5" PRIDX " %5" PRIDX "] [%" PRIDX "] [", graph->gnvtxs, GlobalSESum(ctrl, graph->nedges), GlobalSEMin(ctrl, graph->nvtxs), GlobalSEMax(ctrl, graph->nvtxs), ctrl->CoarsenTo);
        for(i = 0; i < ncon; i++)
       ...
       (graph->finer != NULL &&
        ...
        if(graph->ncon > 1 && graph->level < 3) {
            for(i = 0; i < ncon; i++) {
                ...
                if(ftmp != 0.0)
                    lbvec[i] = (real_t)(nparts)*graph->gnpwgts[rargmax_strd(nparts, graph->gnpwgts + i, ncon) * ncon + i] / ftmp;
                else
                    lbvec[i] = 1.0;
            }
