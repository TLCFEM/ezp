...
void ProjectPartition(ctrl_t* ctrl, graph_t* graph) {
    ...
    if(graph->match_type == PARMETIS_MTYPE_GLOBAL) { /* Only if global matching is on */
        ...
        for(i = 0; i < nnbrs; i++) {
            if(slens[i + 1] - slens[i] > 0) /* Issue a receive only if you are getting something */
                gkMPI_Irecv((void*)(scand + slens[i]), 2 * (slens[i + 1] - slens[i]), IDX_T, peind[i], 1, ctrl->comm, ctrl->rreq + i);
        }
        ...
        for(i = 0; i < nnbrs; i++) {
            if(rlens[i + 1] - rlens[i] > 0) /* Issue a send only if you are sending something */
                gkMPI_Isend((void*)(rcand + rlens[i]), 2 * (rlens[i + 1] - rlens[i]), IDX_T, peind[i], 1, ctrl->comm, ctrl->sreq + i);
        }
    }
    ...
    if(graph->match_type == PARMETIS_MTYPE_GLOBAL) { /* Only if global matching is on */
        ...
        for(i = 0; i < slens[nnbrs]; i++) {
            PASSERTP(ctrl, scand[i].key - firstvtx >= 0 && scand[i].key - firstvtx < graph->nvtxs, (ctrl, "%" PRIDX " %" PRIDX " %" PRIDX "\n", scand[i].key, firstvtx, graph->nvtxs));
...
void ComputePartitionParams(ctrl_t* ctrl, graph_t* graph) {
    ...
    rprintf(ctrl, "where sum: %" PRIDX " %" PRIDX " %" PRIDX " %" PRIDX " %" PRIDX "\n", GlobalSESum(ctrl, nvtxs + graph->nrecv), GlobalSESum(ctrl, isum(nvtxs + graph->nrecv, where, 1)), GlobalSESum(ctrl, isum(nvtxs + 1, xadj, 1)), GlobalSESum(ctrl, isum(graph->nedges, adjncy, 1)), GlobalSESum(ctrl, isum(graph->nedges, adjwgt, 1)));
    ...
    gkMPI_Allreduce((void*)lnpwgts, (void*)gnpwgts, ctrl->nparts * ncon, REAL_T, MPI_SUM, ctrl->comm);
...
void KWayFM(ctrl_t* ctrl, graph_t* graph, idx_t npasses) {
    ...
    for(nswaps = 0, pass = 0; pass < npasses; pass++) {
        ...
        for(c = 0; c < 2; c++) {
            ...
            /******************************************/
            gkMPI_Allreduce((void*)lnpwgts, (void*)pgnpwgts, nparts * ncon, REAL_T, MPI_SUM, ctrl->comm);
            ...
            for(j = 0; j < nparts; j++) {
                for(h = 0; h < ncon; h++) {
                    ...
                    PASSERTP(ctrl, ognpwgts[j * ncon + h] <= badmaxpwgt[j * ncon + h] || pgnpwgts[j * ncon + h] <= ognpwgts[j * ncon + h], (ctrl, "%.4" PRREAL " %.4" PRREAL " %.4" PRREAL "\n", ognpwgts[j * ncon + h], badmaxpwgt[j * ncon + h], pgnpwgts[j * ncon + h]));
                }
            }
            ...
            /* Issue the receives first */
            for(i = 0; i < nnbrs; i++) {
                gkMPI_Irecv((void*)(rupdate + sendptr[i]), sendptr[i + 1] - sendptr[i], IDX_T, peind[i], 1, ctrl->comm, ctrl->rreq + i);
            }
        ...
        IFSET(ctrl->dbglvl, DBG_RMOVEINFO, rprintf(ctrl, "\t\tcut: %" PRIDX "\n", graph->mincut));
