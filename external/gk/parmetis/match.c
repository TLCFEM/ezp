...
void Match_Global0(ctrl_t* ctrl, graph_t* graph) {
    ...
    for(nmatched = pass = 0; pass < NMATCH_PASSES; pass++) {
        ...
        for(i = 0; i < nnbrs; i++) {
            gkMPI_Irecv((void*)(match_granted + recvptr[i]), 2 * (recvptr[i + 1] - recvptr[i]), IDX_T, peind[i], 1, ctrl->comm, ctrl->rreq + i);
        }
        ...
        for(j = i = 0; i < nnbrs; i++) {
            ...
            gkMPI_Isend((void*)(match_requests + j), 2 * (k - j), IDX_T, peind[i], 1, ctrl->comm, ctrl->sreq + i);
            j = k;
        }
        ...
        for(i = 0; i < nnbrs; i++) {
            gkMPI_Irecv((void*)(match_requests + sendptr[i]), 2 * (sendptr[i + 1] - sendptr[i]), IDX_T, peind[i], 1, ctrl->comm, ctrl->rreq + i);
        }

        /* Issue the sends next. */
        for(i = 0; i < nnbrs; i++) {
            gkMPI_Isend((void*)(match_granted + recvptr[i]), 2 * nreqs_pe[i], IDX_T, peind[i], 1, ctrl->comm, ctrl->sreq + i);
        }
        ...
        CommChangedInterfaceData(ctrl, graph, nchanged, changed, match, match_requests, match_granted);
    }
...
void CreateCoarseGraph_Global(ctrl_t* ctrl, graph_t* graph, idx_t cnvtxs) {
    ...
    for(l = i = 0; i < nnbrs; i++) {
        ...
        if(rlens[i + 1] - rlens[i] > 0) {
            gkMPI_Irecv((void*)(rgraph + l), (4 + ncon) * (rlens[i + 1] - rlens[i]) + 2 * rsizes[i], IDX_T, peind[i], 1, ctrl->comm, ctrl->rreq + i);
    ...
    for(ll = l = i = 0; i < nnbrs; i++) {
        if(slens[i + 1] - slens[i] > 0) { /* Issue a send only if you are sending something */
            for(k = slens[i]; k < slens[i + 1]; k++) {
                ...
                sgraph[ll++] = (ctrl->partType == STATIC_PARTITION || ctrl->partType == ORDER_PARTITION ? -1 : vsize[ii]);
                sgraph[ll++] = (ctrl->partType == STATIC_PARTITION || ctrl->partType == ORDER_PARTITION ? -1 : home[ii]);
...
void DropEdges(ctrl_t* ctrl, graph_t* graph) {
    ...
    idx_t *xadj, *adjncy, *adjwgt, *imap, *unmatched;
    ...
    gk_free((void**)&graph->unmatched, LTERM);
