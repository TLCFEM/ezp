 ...
 * serially.
 **************************************************************************/
idx_t Mc_Diffusion(ctrl_t* ctrl, graph_t* graph, idx_t* vtxdist, idx_t* where, idx_t* home, idx_t npasses) {
    idx_t h, i, j;
    ...
    for(pass = 0; pass < npasses; pass++) {
        ...
        while(nvisited < nlinks) {
            ...
            if(proc2sub[mype * 2] != UNMATCHED) {
                ...
                if(egraph != NULL) {
                    ...
                    if(ncon <= 4) {
                        ...
                        nswaps = BalanceMyLink(ctrl, egraph, ehome, me, you, diff_flows, maxdiff, &diff_cost, &diff_lbavg, 1.0 / (real_t)nvtxs);
            ...
            gkMPI_Allgatherv((void*)&sbuffer[rdispl[mype]], rcount[mype], IDX_T, (void*)rbuffer, rcount, rdispl, IDX_T, ctrl->comm);
            ...
            /* Do other stuff */
            gkMPI_Allreduce((void*)visited, (void*)gvisited, matrix.nnzs, IDX_T, MPI_MAX, ctrl->comm);
        ...
        for(h = 0; h < ncon; h++) {
            lbvec[h] = (real_t)(nparts)*npwgts[rargmax_strd(nparts, npwgts + h, ncon) * ncon + h];
        }
 ...
 **************************************************************************/
graph_t* ExtractGraph(ctrl_t* ctrl, graph_t* graph, idx_t* indicator, idx_t* map, idx_t* rmap) {
    idx_t h, i, j;
