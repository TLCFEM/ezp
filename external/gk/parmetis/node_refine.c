...
void AllocateNodePartitionParams(ctrl_t* ctrl, graph_t* graph) {
    ...
    graph->nrinfo = (NRInfoType*)gk_malloc(sizeof(NRInfoType) * nvtxs, "AllocateNodePartitionParams: rinfo");
...
void KWayNodeRefine_Greedy(ctrl_t* ctrl, graph_t* graph, idx_t npasses, real_t ubfrac) {
    ...
    IFSET(ctrl->dbglvl, DBG_REFINEINFO, PrintNodeBalanceInfo(ctrl, nparts, gpwgts, badmaxpwgt, "K-way sep-refinement"));
    ...
    for(pass = 0; pass < npasses; pass++) {
        ...
        for(side = 0; side < 2; side++) {
            ...
            IFSET(ctrl->dbglvl, DBG_REFINEINFO, PrintNodeBalanceInfo(ctrl, nparts, gpwgts, badmaxpwgt, title));
...
/*! This function prints balance information for the parallel k-section refinement
    algorithm. */
/************************************************************************************/
void PrintNodeBalanceInfo(ctrl_t* ctrl, idx_t nparts, idx_t* gpwgts, idx_t* badmaxpwgt, char* title) {
    idx_t i;
