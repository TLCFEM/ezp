...
/***********************************************************************************/
int ParMETIS_V3_NodeND(idx_t* vtxdist, idx_t* xadj, idx_t* adjncy, idx_t* numflag, idx_t* options, idx_t* order, idx_t* sizes, MPI_Comm* comm) {
    idx_t status;
    ...
    ParMETIS_V32_NodeND(vtxdist, xadj, adjncy,
                        ...
                        /*dbglvl=*/(options == NULL || options[0] == 0 ? NULL : &dbglvl), order, sizes, comm);
...
*/
/***********************************************************************************/
int ParMETIS_V32_NodeND(idx_t* vtxdist, idx_t* xadj, idx_t* adjncy, idx_t* vwgt, idx_t* numflag, idx_t* mtype, idx_t* rtype, idx_t* p_nseps, idx_t* s_nseps, real_t* ubfrac, idx_t* seed, idx_t* idbglvl, idx_t* order, idx_t* sizes, MPI_Comm* comm) {
    ...
    ctrl->CoarsenTo = gk_min(graph->gnvtxs - 1, gk_max(1500 * npes, graph->gnvtxs / (5 * NUM_INIT_MSECTIONS * npes)));
...
void Order_Partition(ctrl_t* ctrl, graph_t* graph, idx_t* nlevels, idx_t clevel) {
    ...
    IFSET(ctrl->dbglvl, DBG_PROGRESS, rprintf(ctrl, "[%6" PRIDX " %8" PRIDX " %5" PRIDX " %5" PRIDX "][%" PRIDX "][%" PRIDX "]\n", graph->gnvtxs, GlobalSESum(ctrl, graph->nedges), GlobalSEMin(ctrl, graph->nvtxs), GlobalSEMax(ctrl, graph->nvtxs), ctrl->CoarsenTo, GlobalSEMax(ctrl, imax(graph->nvtxs, graph->vwgt, 1))));
...
/*********************************************************************************/
void LabelSeparators(ctrl_t* ctrl, graph_t* graph, idx_t* lastnode, idx_t* perm, idx_t* order, idx_t* sizes) {
...
void CompactGraph(ctrl_t* ctrl, graph_t* graph, idx_t* perm) {
    ...
    gkMPI_Allgather((void*)&cnvtxs, 1, IDX_T, (void*)cvtxdist, 1, IDX_T, ctrl->comm);
    ...
    gk_free((void**)&graph->match, (void**)&graph->cmap, (void**)&graph->lperm, (void**)&graph->where, (void**)&graph->label, (void**)&graph->ckrinfo, (void**)&graph->nrinfo, (void**)&graph->lpwgts, (void**)&graph->gpwgts, (void**)&graph->sepind, (void**)&graph->peind, (void**)&graph->sendptr, (void**)&graph->sendind, (void**)&graph->recvptr, (void**)&graph->recvind, (void**)&graph->imap, (void**)&graph->rlens, (void**)&graph->slens, (void**)&graph->rcand, (void**)&graph->pexadj, (void**)&graph->peadjncy, (void**)&graph->peadjloc, LTERM);
