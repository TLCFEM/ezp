 ...
 ************************************************************************************/
int ParMETIS_SerialNodeND(idx_t* vtxdist, idx_t* xadj, idx_t* adjncy, idx_t* numflag, idx_t* options, idx_t* order, idx_t* sizes, MPI_Comm* comm) {
    idx_t i, npes, mype;
    ctrl_t* ctrl = NULL;
    graph_t* agraph = NULL;
    ...
    if(mype == 0) {
        ...
        METIS_NodeNDP(agraph->nvtxs, agraph->xadj, agraph->adjncy, agraph->vwgt, npes, NULL, perm, iperm, sizes);
    }
    ...
    gk_free((void**)&agraph->xadj, &agraph->adjncy, &perm, &iperm, &sendcount, &displs, &agraph, LTERM);
...
graph_t* AssembleEntireGraph(ctrl_t* ctrl, idx_t* vtxdist, idx_t* xadj, idx_t* adjncy) {
    ...
    gkMPI_Gatherv((void*)xadj, nvtxs, IDX_T, axadj, recvcounts, displs, IDX_T, 0, ctrl->comm);
