...
void PartitionSmallGraph(ctrl_t* ctrl, graph_t* graph) {
    ...
    METIS_PartGraphKway(&agraph->nvtxs, &ncon, agraph->xadj, agraph->adjncy, agraph->vwgt, NULL, agraph->adjwgt, &nparts, ctrl->tpwgts, NULL, moptions, &graph->mincut, mypart);
    ...
    gkMPI_Scatterv((void*)mypart, sendcounts, displs, IDX_T, (void*)graph->where, graph->nvtxs, IDX_T, 0, ctrl->comm);
