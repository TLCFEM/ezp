...
void printGbisect(gbisect_t* Gbisect) {
    ...
    G = Gbisect->G;
    printf("\n#nodes %d, #edges %d, totvwght %d\n", G->nvtx, G->nedges >> 1, G->totvwght);
    printf("partition weights: S %d, B %d, W %d\n", Gbisect->cwght[GRAY], Gbisect->cwght[BLACK], Gbisect->cwght[WHITE]);
    for(u = 0; u < G->nvtx; u++) {
        count = 0;
        printf("--- adjacency list of node %d (weight %d, color %d)\n", u, G->vwght[u], Gbisect->color[u]);
        istart = G->xadj[u];
        istop = G->xadj[u + 1];
...
void checkSeparator(gbisect_t* Gbisect) {
    ...
    if((checkS != cwght[GRAY]) || (checkB != cwght[BLACK]) || (checkW != cwght[WHITE])) {
        printf("ERROR in partitioning: checkS %d (S %d), checkB %d (B %d), "
               "checkW %d (W %d)\n",
               checkS, cwght[GRAY], checkB, cwght[BLACK], checkW, cwght[WHITE]);
        err = TRUE;
    }
    if(err) quit();
}
...
void constructSeparator(gbisect_t* Gbisect, options_t* options, timings_t* cpus) {
    ...
    if(options[OPTION_MSGLVL] > 2)
        printf("\t %2d. dom.dec. sep.: S %d, B %d, W %d [cost %7.2f]\n", i, dd->cwght[GRAY], dd->cwght[BLACK], dd->cwght[WHITE], F(dd->cwght[GRAY], dd->cwght[BLACK], dd->cwght[WHITE]));
...
******************************************************************************/
PORD_INT
smoothBy2Layers(gbisect_t* Gbisect, PORD_INT* bipartvertex, PORD_INT* pnX, PORD_INT black, PORD_INT white) {
    gbipart_t* Gbipart;
    ...
    printf("Dulmage-Mendelsohn decomp. computed\n"
           ...
           dmwght[SI], dmwght[SX], dmwght[SR], dmwght[BI], dmwght[BX], dmwght[BR]);
#endif
...
void smoothSeparator(gbisect_t* Gbisect, options_t* options) {
    ...
    do { /* ---------------------------------------------------------------
        ...
        if((options[OPTION_MSGLVL] > 2) && (a))
            printf("\t separator smoothed: S %d, B %d, W %d [cost %7.2f]\n", cwght[GRAY], cwght[BLACK], cwght[WHITE], F(cwght[GRAY], cwght[BLACK], cwght[WHITE]));
    } while(a);
