...
SPACE_ordering(graph_t* G, options_t* options, timings_t* cpus) {
    ...
    options_t default_options[] = {SPACE_ORDTYPE, SPACE_NODE_SELECTION1, SPACE_NODE_SELECTION2, SPACE_NODE_SELECTION3, SPACE_DOMAIN_SIZE, SPACE_MSGLVL};
...
******************************************************************************/
void SPACE_solve(inputMtx_t* A, FLOAT* rhs, FLOAT* xvec, options_t* options, timings_t* cpus) {
    graph_t* G;
    elimtree_t *T, *T2;
    factorMtx_t* L;
    timings_t cpusOrd[ORD_TIME_SLOTS], cpusFactor[NUMFAC_TIME_SLOTS];
    timings_t t_graph, t_ord, t_etree, t_symb, t_num, t_solvetri;
    options_t default_options[] = {SPACE_ORDTYPE, SPACE_NODE_SELECTION1, SPACE_NODE_SELECTION2, SPACE_NODE_SELECTION3, SPACE_DOMAIN_SIZE, SPACE_MSGLVL, SPACE_ETREE_NONZ};
    ...
    if(options[OPTION_MSGLVL] > 0)
        printf("quality of initial elim. tree: #fronts %d, #indices %d\n\t"
               ...
               T->nfronts, nFactorIndices(T), nFactorEntries(T), nFactorOps(T), nWorkspace(T));
    ...
    if(options[OPTION_MSGLVL] > 0)
        printf("quality of transformed elim. tree: #fronts %d, #indices %d\n\t"
               "nzl %d, ops %e, wspace %d\n",
               T2->nfronts, nFactorIndices(T2), nFactorEntries(T2), nFactorOps(T2), nWorkspace(T2));
...
******************************************************************************/
void SPACE_solveWithPerm(inputMtx_t* A, PORD_INT* perm, FLOAT* rhs, FLOAT* xvec, options_t* options, timings_t* cpus) {
    graph_t* G;
    elimtree_t *T, *T2;
    factorMtx_t* L;
...
SPACE_mapping(graph_t* G, PORD_INT* perm, options_t* options, timings_t* cpus) {
    ...
    options_t default_options[] = {SPACE_MSGLVL, SPACE_ETREE_NONZ, SPACE_ETREE_BAL, 2};
