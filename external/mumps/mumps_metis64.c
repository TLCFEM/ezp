...
#endif
void MUMPS_CALL
MUMPS_PARMETIS_64(MUMPS_INT8* first, MUMPS_INT8* vertloctab, MUMPS_INT8* edgeloctab,
#if defined(parmetis3)
                  MUMPS_INT* numflag,
                  MUMPS_INT* options,
#else
...
void MUMPS_CALL
MUMPS_PARMETIS_VWGT_64(MUMPS_INT8* first, MUMPS_INT8* vertloctab, MUMPS_INT8* edgeloctab,
#if defined(parmetis3)
                       MUMPS_INT* numflag,
                       MUMPS_INT* options,
#else
...
void MUMPS_CALL
MUMPS_METIS_KWAY_64(MUMPS_INT8* n, MUMPS_INT8* iptr, MUMPS_INT8* jcn, MUMPS_INT8* k, MUMPS_INT8* part)
...
/* SELECTIVE I8 FIXME: add an argument *ierr, check it on exit */
{
    ...
    METIS_PartGraphKway(&nINT, iptr, jcn, NULL, NULL, &wgtflag, &numflag, &kINT, options, &edgecut, part);
#else /* METIS >= 5 */
    int ierr;
#if (IDXTYPEWIDTH == 64)
...
void MUMPS_CALL
MUMPS_METIS_KWAY_AB_64(MUMPS_INT8* n, MUMPS_INT8* iptr, MUMPS_INT8* jcn, MUMPS_INT8* k, MUMPS_INT8* part, MUMPS_INT8* vwgt)
/* n     -- the size of the graph to be partitioned
   iptr  -- pointer to the beginning of each node's adjacency list
   jcn   -- jcn[iptr[i]:iptr[i+1]-1] contains the list of neighbors of node i
   k     -- the number of parts
...
/* SELECTIVE I8 FIXME: add an argument *ierr, check it on exit */
{
    ...
    kINT = (MUMPS_INT)(*k);
    /* void METIS_PartGraphKway(int *, idxtype *, idxtype *, idxtype *, idxtype *, int *, int *, int *, int *, int *, idxtype *); */
    METIS_PartGraphKway(&nINT, iptr, jcn, vwgt, NULL, &wgtflag, &numflag, &kINT, options, &edgecut, part);
#else /* METIS >= 5 */
    int ierr;
#if (IDXTYPEWIDTH == 64)
    MUMPS_INT8 ncon, edgecut, options[METIS_NOPTIONS];
