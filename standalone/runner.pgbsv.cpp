 ...
 ******************************************************************************/
/**
 * @brief Example caller to the standalone `pgbsv` solver.
 *
 * @author tlc
 * @date 07/03/2025
 * @version 1.0.0
 * @file runner.pgbsv.cpp
 * @{
 */
...
int main(int argc, char* argv[]) {
    ...
    const auto worker = comm_world.spawn(0, NUM_NODE, {"solver.pgbsv"});
    const auto all = mpl::communicator(worker, mpl::communicator::order_low);

    constexpr auto N = 6, KL = 1, KU = 1, NRHS = 1;

    int config[]{N, KL, KU, NRHS, 1};

    std::vector<double> A(N * (2 * (KL + KU) + 1), 0.), B(N * NRHS, 1.);
