...
template<ezp::data_t DT, ezp::index_t IT> int run(const int N, const int KL, const int KU, const int NRHS) {
    std::vector<DT> A, B;
    ...
    const auto error = ezp::pdbsv<DT, IT>().solve({N, N, KL, KU, A.data()}, {N, NRHS, B.data()});
...
int main(int, char**) {
    ...
    if(FLOAT >= 10) return run<complex16, int_t>(N, KL, KU, NRHS);
    if(FLOAT >= 0) return run<double, int_t>(N, KL, KU, NRHS);
    if(FLOAT > -10) return run<float, int_t>(N, KL, KU, NRHS);

    return run<complex8, int_t>(N, KL, KU, NRHS);
}
