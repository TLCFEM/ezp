...
int main(int argc, char** argv) {
    ...
    if(FLOAT >= 10) return run<complex16>(N, KLU, NRHS);
    if(FLOAT >= 0) return run<double>(N, KLU, NRHS);
    if(FLOAT > -10) return run<float>(N, KLU, NRHS);

    return run<complex8>(N, KLU, NRHS);
}
