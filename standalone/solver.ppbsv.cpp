...
int main(int, char**) {
    ...
    if(FLOAT >= 10) return run<complex16, int_t>(N, KLU, NRHS);
    if(FLOAT >= 0) return run<double, int_t>(N, KLU, NRHS);
    if(FLOAT > -10) return run<float, int_t>(N, KLU, NRHS);

    return run<complex8, int_t>(N, KLU, NRHS);
}
