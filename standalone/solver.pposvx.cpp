...
int main(int, char**) {
    ...
    if(FLOAT >= 10) return run<complex16, int_t, ezp::pposvx<complex16, int_t>>(N, NRHS);
    if(FLOAT >= 0) return run<double, int_t, ezp::pposvx<double, int_t>>(N, NRHS);
    if(FLOAT > -10) return run<float, int_t, ezp::pposvx<float, int_t>>(N, NRHS);

    return run<complex8, int_t, ezp::pposvx<complex8, int_t>>(N, NRHS);
}
