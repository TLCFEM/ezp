...
class MasterConsoleReporter : public ConsoleReporter {
...
auto benchmark_mumps(const int_t n, const int_t nnz, std::vector<int_t>& row, std::vector<int_t>& col, std::vector<flt_t>& val) {
    std::vector b(n, flt_t{1});
    auto solver = ezp::mumps<flt_t, int_t>(ezp::unsymmetric);

    solver.icntl_printing_level(3);
...
TEST_CASE("MUMPS Benchmark", "[Benchmarking]") {
